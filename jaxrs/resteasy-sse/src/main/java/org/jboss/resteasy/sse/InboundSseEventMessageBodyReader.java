...
public class InboundSseEventMessageBodyReader implements MessageBodyReader<InboundSseEvent> {
   ...
   public boolean isReadable(Class<?> type, Type genericType,
      ...
      return InboundSseEvent.class.isAssignableFrom(type) && SseFeature.SERVER_SENT_EVENTS_TYPE.isCompatible(mediaType);
   }


   @Override
   public InboundSseEvent readFrom(final Class<InboundSseEvent> type,
      ...
      final String charsetName = getCharset(mediaType).name();
      final InboundSseEventImpl.Builder eventBuilder =
         new InboundSseEventImpl.Builder(annotations, mediaType, headers);
   ...
   private void processField(final InboundSseEventImpl.Builder inboundEventBuilder, final String name,
      ...
      final String valueString = new String(value, getCharset(mediaType));
      if ("event".equals(name)) {
         ...
         try {
            ...
            LogMessages.LOGGER.inEventRetryParseError(valueString, ex);
         }
      } else {
         LogMessages.LOGGER.inEventFieldNotRecognizedWarning(name, valueString);
      }
   }
