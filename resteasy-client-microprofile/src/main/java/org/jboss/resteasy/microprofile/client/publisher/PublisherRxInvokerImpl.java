...
public class PublisherRxInvokerImpl implements PublisherRxInvoker {
    ...
    public PublisherRxInvokerImpl(final SyncInvoker syncInvoker, final ExecutorService executorService) {
        if (!(syncInvoker instanceof ClientInvocationBuilder)) {
            // todo rls  throw new ProcessingException(Messages.MESSAGES.expectedClientInvocationBuilder(syncInvoker.getClass().getName()));
            throw new ProcessingException("ExpectedClientInvocationbuilder: " + syncInvoker.getClass().getName());
        }
    ...
    /***
     @Override public BackpressureStrategy getBackpressureStrategy() {
     return backpressureStrategy;
     }

     @Override public void setBackpressureStrategy(BackpressureStrategy backpressureStrategy) {
     this.backpressureStrategy = backpressureStrategy;
     }
     *****/
    private <T> Publisher<T> eventSourceToObservable(SseEventSourceImpl sseEventSource, Class<T> clazz, String verb, Entity<?> entity, MediaType[] mediaTypes) {
    ...
    private <T> Publisher<T> eventSourceToObservable(SseEventSourceImpl sseEventSource, GenericType<T> type, String verb, Entity<?> entity, MediaType[] mediaTypes) {
        Publisher<T> flowable = Flowable.create(
                new FlowableOnSubscribe<T>() {
