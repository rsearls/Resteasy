 ...
 */
public class ManualClosingApacheHttpClient5Engine implements ApacheHttpClientEngine
{
   ...
   protected HttpContext httpContextProvider;
   ...
   public ManualClosingApacheHttpClient5Engine()
   {
      this.httpClient = createDefaultHttpClient();
      this.allowClosingHttpClient = true;
   }

   public ManualClosingApacheHttpClient5Engine(final HttpHost defaultProxy)
   {
   ...
   public ManualClosingApacheHttpClient5Engine(final HttpClient httpClient)
   {
      this.httpClient = httpClient;
      this.allowClosingHttpClient = true;
   }

   public ManualClosingApacheHttpClient5Engine(final HttpClient httpClient, final boolean closeHttpClient)
   {
   ...
   public ManualClosingApacheHttpClient5Engine(final HttpClient httpClient, final HttpContext httpContextProvider)
   {
      this.httpClient = httpClient;
   ...
   public Response invoke(Invocation inv)
   {
      ...
      try
      {
         ...
         if (System.getSecurityManager() == null) {
            res = httpClient.execute(httpMethod, httpContextProvider);
         } else {
            try {
               res = AccessController.doPrivileged(new PrivilegedExceptionAction<HttpResponse>() {
                  @Override
                  public HttpResponse run() throws Exception {
                     return httpClient.execute(httpMethod, httpContextProvider);
                  }
               });
            } catch (PrivilegedActionException pae) {
               throw new RuntimeException(pae);
      ...
      ClientResponse response = new FinalizedClientResponse(request.getClientConfiguration(), request.getTracingLogger())
      {
         ...
         public InputStream getInputStream()
         {
            if (stream == null)
            {
               HttpEntity entity = ((BasicClassicHttpResponse)res).getEntity();
               if (entity == null)
                  return null;
               try
               {
   ...
   protected HttpUriRequestBase createHttpMethod(String url, String restVerb)
   {
   ...
   protected void loadHttpMethod(final ClientInvocation request, HttpUriRequestBase httpMethod) throws Exception
   {
   ...
   protected void commitHeaders(ClientInvocation request, HttpUriRequestBase httpMethod)
   {
    ...
    */
   protected void cleanUpAfterExecute(final HttpUriRequestBase httpMethod)
   {
   ...
   protected HttpEntity buildEntity(final ClientInvocation request) throws IOException
   {
      ...
      if (mediaType != null) {
         tmpContentType = ContentType.create(mediaType.toString());
      }
   ...
   protected void setRedirectRequired(final ClientInvocation request, final HttpUriRequestBase httpMethod)
   {
   ...
   protected void setRedirectNotRequired(final ClientInvocation request, final HttpUriRequestBase httpMethod)
   {
      RequestConfig.Builder requestBuilder = RequestConfig.copy(getCurrentConfiguration(request, httpMethod));
   ...
   private RequestConfig getCurrentConfiguration(final ClientInvocation request, final HttpUriRequestBase httpMethod)
   {
      ...
      {
         ManualClosingApacheHttpClient5Engine engine =
                 ((ManualClosingApacheHttpClient5Engine) request.getClient().httpEngine());
