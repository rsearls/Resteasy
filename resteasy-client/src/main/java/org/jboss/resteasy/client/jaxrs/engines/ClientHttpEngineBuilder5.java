...
public class ClientHttpEngineBuilder5 implements ClientHttpEngineBuilder {

    private ResteasyClientBuilder that;

    @Override
    public ClientHttpEngineBuilder resteasyClientBuilder(
            ResteasyClientBuilder resteasyClientBuilder)
    {
        that = resteasyClientBuilder;

        // apache 5.x appears to need 1.5 sec for socketTimeout
        if (that.getReadTimeout(TimeUnit.MILLISECONDS) == -1) {
            that.readTimeout(1005000, TimeUnit.MILLISECONDS);
        }
    ...
    public ClientHttpEngine build()
    {
        ...
        try
        {
            ...
            Iterator clientConfigProviderIterator = ServiceLoader.load(
                    ClientConfigProvider.class).iterator();
            ...
            if (that.getConnectionPoolSize() > 0)
            {
                PoolingHttpClientConnectionManager tcm = new PoolingHttpClientConnectionManager(
                        ...
                        TimeValue.of(that.getConnectionTTL(TimeUnit.MILLISECONDS),TimeUnit.MILLISECONDS),
                        null,
                        null,
                        null);
            ...
            if (that.getReadTimeout(TimeUnit.MILLISECONDS) > -1)
            {
                rcBuilder.setDefaultKeepAlive(that.getReadTimeout(TimeUnit.MILLISECONDS),
                        TimeUnit.MILLISECONDS);
            }
            if (that.getConnectionTimeout(TimeUnit.MILLISECONDS) > -1)
            {
                rcBuilder.setConnectTimeout(that.getConnectionTimeout(TimeUnit.MILLISECONDS),
                        TimeUnit.MILLISECONDS);
            }
            if (that.getConnectionCheckoutTimeout(TimeUnit.MILLISECONDS) > -1)
            {
                rcBuilder.setConnectionRequestTimeout(that.getConnectionCheckoutTimeout(TimeUnit.MILLISECONDS),
                        TimeUnit.MILLISECONDS);
            }

            return createEngine(cm, rcBuilder, getDefaultProxy(that),
                    that.getResponseBufferSize(), verifier, theContext);
        }
        catch (Exception e)
        {
            throw new RuntimeException(e);
    ...
    private static HttpHost getDefaultProxy(ResteasyClientBuilder that) {
        ...
        return hostName != null ? new HttpHost(that.getDefaultProxyScheme(),
                hostName, that.getDefaultProxyPort()) : null;
    }
                                            ...
                                            final HttpHost defaultProxy, final int responseBufferSize, final HostnameVerifier verifier, final SSLContext theContext)
    {
        ...
        ApacheHttpClient5Engine engine = new ApacheHttpClient5Engine(httpClient, true);
